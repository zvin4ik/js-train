...
function findElementGreaterThan(arr, num) {
  ...
  if (Array.isArray(arr)) {
    const findElem = arr.find(element => element > num);
    return findElem;
  } else {
    return "undefined";
  }
...
function findIndexByProperty(arr, name) {
  ...
  if (Array.isArray(arr)) {
    const findName = arr.findIndex(index => index.name === name);
...
function findLastEvenNumber(arr) {
  ...
  if (Array.isArray(arr)){
    const findNum = arr.findLast(element => element % 2 === 0);
    return (findNum !== undefined) ? findNum : 'Парних чисел не знайдено';
  } else {
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  // Повертаємо рядок.
  return (Array.isArray(arr)) ? arr.slice(3).join(' ') : 'Вхідний параметр має бути масивом';
}
...
function getArrayKeys(arr) {
  ...
  if (Array.isArray(arr)) {
    const includeKey = arr.keys();    
    const arrayKeys = Array.from(includeKey);
    return arrayKeys;    
  } else {
    return 'Вхідний параметр має бути масивом'
  }
...
function getPositiveArrayValues(arr) {
  ...
  if (Array.isArray(arr)) {
    ...
    const convertArr = Array.from(getValues);
    const positiveArrayValues = convertArr.filter(convertArr => convertArr > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  if (Array.isArray(arr)) {
    const delItem = arr.splice(0, startIdx);
    const mapArr = delItem.map(delItem => delItem * 2);
    return mapArr;
  } else 
    return 'Вхідний параметр має бути масивом';
}
...
function sumAndPrint(arr) {
  ...
  if (Array.isArray(arr)) {
    const sumArr = arr.reduce((acumulator, value) => acumulator + value, 0);
    const printArr = arr.forEach(element => console.log(element))
    return printArr, sumArr;
  } else 
    return 'Вхідний параметр має бути масивом';
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  if (Array.isArray(arr)) {
    ...
    const mapArr = flatArr.map((flatArr) => flatArr **2);
    const filterArr = mapArr.filter((flatArr) => flatArr > threshold);
    return filterArr;
  } else 
    return 'Вхідний параметр має бути масивом';
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (Array.isArray(arr)) {
    ...
    const sum = sliceArr.reduceRight((acumulator, value) => acumulator + value, 0);
...
function insertElementAt(arr, index, element) {
  ...
  return (Array.isArray(arr)) ? (arr.splice(index, 0, element), arr) : 'Вхідний параметр має бути масивом';
}
...
return (Array.isArray(arr)) ? arr.flat(Infinity) : 'Вхідний параметр має бути масивом';
}
...
function findLongestWord(arr) {
  ...
  if (Array.isArray(arr)){
  const longestWord = arr.reduce((long, current) => {
...
function calculateTotalPrice(arr) {
  ...
  if (Array.isArray(arr)) {
    const totalPrice = arr.reduce((element, index) => element + index.price, 0);
    return totalPrice;
  } else {
    return 0;
  }
...
function findLastNegativeNumberIndex(arr) {
  ...
  if (Array.isArray(arr)) {
    const lastIndex = arr.findLastIndex((element, index) => element < 0);
    return lastIndex;
  } else {
    return -1;
  }
