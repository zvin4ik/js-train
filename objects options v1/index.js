...
console.log(checkProperty({ a: 1, b: 2, c: 3 }, "a")); // Виведе true
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const frezze = Object.freeze(merge);
  if (Object.isFrozen(frezze)) {
    return frezze
  } else {
...
const result = arr.filter(value => value[key])
return result
}
...
function getObjectValuesSum(obj) {
  ...
  if(typeof obj !== "object")
    return 0;
  const values = Object.values(obj);
  const summ = values.reduce((key, numder) => key + numder, 0);
  return summ;
}
